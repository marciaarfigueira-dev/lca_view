...
function hydrateFilters(data) {
  ...
  fillSelect(elements.operation, operationOptions(data), "Operation");
...
function refreshOperationOptions() {
  const rows = rowsForOptions({ includeOperation: false, includeEquipment: false });
  const options = operationOptions(rows);
  const allowed = new Set(options.map((option) => option.value));
  if (state.filters.operation !== "all" && !allowed.has(state.filters.operation)) {
    state.filters.operation = "all";
  }
  fillSelect(elements.operation, options, "Operation");
  elements.operation.value = state.filters.operation;
}

function refreshEquipmentOptions() {
  const rows = rowsForOptions({ includeEquipment: false });
  const equipment = uniqueValues(rows, "equipment").sort();
  const allowed = new Set(equipment);
  if (state.filters.equipment !== "all" && !allowed.has(state.filters.equipment)) {
    state.filters.equipment = "all";
  }
  fillSelect(elements.equipment, equipment, "Equipment");
  elements.equipment.value = state.filters.equipment;
}

function rowsForOptions({ includeOperation = true, includeEquipment = true } = {}) {
  return state.data.filter((row) => {
    ...
    if (state.filters.category !== "all" && row.operation_category !== state.filters.category) return false;
    if (includeOperation && state.filters.operation !== "all" && row.operation_normalized !== state.filters.operation) {
      return false;
    }
    if (includeEquipment && state.filters.equipment !== "all" && row.equipment !== state.filters.equipment) return false;
    return true;
  });
}

function operationOptions(rows) {
  return uniqueValues(rows, "operation_normalized")
