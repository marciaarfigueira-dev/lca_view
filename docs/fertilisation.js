...
const NUTRIENTS = [
  { value: "n", label: "Nitrogen (N)", symbol: "N", keyHa: "n_kg_ha_weight", keyT: "n_kg_t" },
  { value: "p", label: "Phosphorus (P)", symbol: "P", keyHa: "p_kg_ha_weight", keyT: "p_kg_t" },
  { value: "k", label: "Potassium (K)", symbol: "K", keyHa: "k_kg_ha_weight", keyT: "k_kg_t" },
  { value: "so4", label: "Sulphate (SO₄)", symbol: "SO₄", keyHa: "so4_kg_ha_weight", keyT: "so4_kg_t" },
];
...
const elements = {
  ...
  nutrient: document.getElementById("nutrient-filter"),
...
function enrichRecord(row) {
  return {
    ...row,
    area_ha: row.covered_area ?? row.area_TOTAL ?? 0,
...
function hydrateFilters(data) {
  ...
  fillSelect(
    elements.nutrient,
    NUTRIENTS.filter((nutrient) => data.some((row) => hasNutrient(row, nutrient.value))).map((nutrient) => ({
...
function attachEvents() {
  ...
  elements.nutrient.addEventListener("change", () => {
    state.filters.nutrient = elements.nutrient.value;
    render();
  });
...
function applyFilters(rows, filters) {
  return rows.filter((row) => {
    ...
    if (filters.nutrient !== "all" && !hasNutrient(row, filters.nutrient)) return false;
    if (filters.search) {
...
function renderActiveFilters(filters, count) {
  ...
  if (filters.nutrient !== "all") active.push(nutrientLabel(filters.nutrient));
...
function renderStats(rows) {
  ...
  const nutrients = selectedNutrients(state.filters.nutrient).map((nutrient) => {
    return {
      nutrient: nutrient.symbol,
      ha: average(rows, nutrient.keyHa),
      tonne: average(rows, nutrient.keyT),
    };
  });
  const stats = [
    { label: "Events", value: formatNumber(rows.length, 0) },
    { label: "Area Covered (ha)", value: formatNumber(totalArea, 1) },
...
function renderPivot(rows) {
  const nutrients = selectedNutrients(state.filters.nutrient);
  if (!rows.length) {
    ...
    <table>
      <thead>
        <tr>
          ...
          ${nutrients.map((nutrient) => `<th>${nutrient.symbol} (kg/t)</th>`).join("")}
          ${nutrients.map((nutrient) => `<th>${nutrient.symbol} (kg/ha)</th>`).join("")}
        </tr>
      </thead>
      <tbody>
        ${grouped
          .map(
            (row) => `
              <tr>
                ...
                ${nutrients.map((nutrient) => `<td>${row[`${nutrient.value}T`] === null ? "—" : formatNumber(row[`${nutrient.value}T`], 2)}</td>`).join("")}
                ${nutrients.map((nutrient) => `<td>${row[`${nutrient.value}Ha`] === null ? "—" : formatNumber(row[`${nutrient.value}Ha`], 1)}</td>`).join("")}
              </tr>
            `
          )
          .join("")}
          ...
          .map(
            (row) => `
              <tr>
                ...
                ${nutrients.map((nutrient) => `<td>${row[nutrient.keyHa] == null ? "—" : formatNumber(row[nutrient.keyHa], 1)}</td>`).join("")}
                ${nutrients.map((nutrient) => `<td>${row[nutrient.keyT] == null ? "—" : formatNumber(row[nutrient.keyT], 2)}</td>`).join("")}
...
function selectedNutrients(value) {
  if (value === "all") return NUTRIENTS;
  return NUTRIENTS.filter((nutrient) => nutrient.value === value);
}

function hasNutrient(row, value) {
  const nutrient = NUTRIENTS.find((item) => item.value === value);
  if (!nutrient) return false;
  return (row[nutrient.keyHa] || 0) > 0 || (row[nutrient.keyT] || 0) > 0;
}

function nutrientLabel(value) {
  const nutrient = NUTRIENTS.find((item) => item.value === value);
  return nutrient ? nutrient.label : value;
}
