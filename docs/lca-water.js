...
function renderImpacts(rows) {
  ...
  rows.forEach((r) => {
    ...
    if (!scalar) return;
    if (!source) return;
    if (source.Total != null) totalAbs += source.Total * scalar;
  ...
  const displayedTotal = entries.reduce((sum, entry) => sum + entry.value, 0);
  const officialTotal = denom ? totalAbs / denom : totalAbs;
  const percentDenominator = officialTotal || displayedTotal;
  ...
  elements.impactBars.innerHTML = entries
    .map((entry, idx) => {
      ...
      const pct = percentDenominator ? (entry.value / percentDenominator) * 100 : 0;
      const pctWidth = Math.max(0, Math.min(100, pct));
      return `
        <div class="bar-row">
          <div class="bar-label">
            <div>${entry.cat}</div>
            <small>${formatNumber(entry.value, 2)} ${unit} (${formatNumber(pct, 1)}% of Total)</small>
          </div>
          <div class="bar-track">
